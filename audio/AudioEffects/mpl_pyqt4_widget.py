...
class MyMplCanvas(FigureCanvas):
	def __init__(self, parent=None, width = 10, height = 12, dpi = 100, sharex = None, sharey = None):
		...
		#self.bx = self.fig.add_subplot(211, sharex = sharex, sharey = sharey)
    		self.ax = self.fig.add_subplot(111, sharex = sharex, sharey = sharey)
		self.fig.subplots_adjust(left=0, bottom=0, right=1, top=1)
           #subplots_adjust(left,right,bottom,top,wsapce,hspace),参数取值范围是0-1，返回它所创建的Axes对象
	...
	def format_labels(self):
		...
		self.ax.title.set_fontsize(0)
		self.ax.set_xlabel(self.xtitle, fontsize = 0)
		self.ax.set_ylabel(self.ytitle, fontsize = 0)
		...
		for ylabel in labels_y:
			ylabel.set_fontsize(0)
			ylabel.set_color('b')

#		self.bx.set_title(self.PlotTitle)
#		self.bx.title.set_fontsize(0)
#		self.bx.set_xlabel(self.xtitle, fontsize = 0)
#		self.bx.set_ylabel(self.ytitle, fontsize = 0)
