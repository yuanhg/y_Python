# -*- coding: utf-8 -*-
"""
Created on Mon Jan 13 10:52:48 2020
...
class Calendar:
    ...
    def __get_calendar(s, locale, fwday):
        ...
        else:  #默认locale=zh_CN , 显示就是你操作系统设置的的月名和周名，而不是英文的Monday, July之类的周名和月名
...
if __name__ == '__main__':
    ...
    date_str = StringVar()
    date = Entry(root, textvariable = date_str)
    ...
    def date_str_gain():
        for date in [Calendar((x, y), 'ur').selection()]:
            if date: date_str.set(date) 
        
    Button(root, text = '日期:', command = date_str_gain).place(x = 0, y = 0, relx = 1/20, rely = 1/6, relwidth = 4/20, relheigh = 2/3)
    #Button(root, text = '日期:', command = date_str_gain).place(x=0, y=0, anchor=NW)
